            ...
            while (!pendingPayments.EoF) {
                try {
                    ...
                    if (payDocument(Company, pendingPayments)) {
                        updateRS.DoQuery("UPDATE PagosCuenta SET INT_Estado = 'P' WHERE IdPago = " + pendingPayments.Fields.Item("IdPago").Value);
                        Company.EndTransaction(SAPbobsCOM.BoWfTransOpt.wf_Commit);
                    } else {
                        if (Company.InTransaction) { Company.EndTransaction(SAPbobsCOM.BoWfTransOpt.wf_RollBack); }
                        updateRS.DoQuery("UPDATE PagosCuenta SET INT_Estado = 'E', INT_Desc = '" + Company.GetLastErrorDescription().Replace('\'',' ') + "' WHERE IdPago = " + pendingPayments.Fields.Item("IdPago").Value);
                    }
                } catch (Exception) {
                    if (Company.InTransaction) {
                        if (Company.InTransaction) { Company.EndTransaction(SAPbobsCOM.BoWfTransOpt.wf_RollBack); }
                        updateRS.DoQuery("UPDATE PagosCuenta SET INT_Estado = 'E' WHERE IdPago = " + pendingPayments.Fields.Item("IdPago").Value);
                    }
                } finally {
